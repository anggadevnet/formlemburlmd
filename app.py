...
def push_to_github(file_path, repo_path, commit_message):
    """Fungsi untuk upload/update file ke GitHub"""
...
# --- FUNGSI DATABASE ---
def init_db():
    # Coba download dari GitHub dulu jika ada (ini otomatis dilakukan streamlit saat pull repo)
    # Jika file lokal tidak ada, buat baru
...
# --- FITUR KALKULATOR")
                    ...
                    st.download_button("📥 Download ZIP", data=zip_buffer, file_name="converted.zip", mime="application/zip")
                except Exception as e: st.error(f"Error: {e}")
...
def show_overtime_calculator():
    ...
    if st.button("Hitung Durasi", type="primary"):
        ...
        if is_weekend:
            ...
            # Potong break 1 jam kalau lembur > 4 jam (asumsi sederhana)
            # Silakan sesuaikan logic CASE 1,2,3,4 kamu di sini jika perlu detail
            st.success(f"TOTAL: {int(delta.total_seconds()//3600)} Jam")
