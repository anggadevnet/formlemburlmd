...
def show_guest_view():
    ...
    if pilih_nama == "Semua": df_show = df_filtered_month
    else: df_show = df_filtered_month[df_filtered_month['Nama'] == pilih_nama]
...
def show_form_content():
    ...
    if st.button("Generate & Save", type="primary"):
        try:
            ...
            st.download_button(
                label="📥 Download Surat Lembur (.docx)", data=buffer, file_name=filename,
